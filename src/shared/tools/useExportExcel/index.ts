...
export const useExportExcel = () => {
  /** 根据配置类型处理单个值 */
  const transformValue = (value: any, config: HeadConfig, typeOptions?: TypeHandleOptions): any => {
    switch (config.type) {
      case 'number':
  ...
  const buildHeaderRow = <T extends AnyValueRecord>(
    ...
    return Object.entries(headsConfig).reduce<Partial<Record<keyof T, string>>>(
      (acc, [key, config]) => {
        if (config) {

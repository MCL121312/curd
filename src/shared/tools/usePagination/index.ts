...
export function usePagination(
  ...
  const changePageSize = (newSize: (typeof pageSizes.value)[number]) => {
    ...
    if (!pageSizes.value.includes(newSize)) {
      console.warn(`pageSize ${newSize} is not in available sizes: ${pageSizes.value.join(', ')}`);
    }
    pagination.value.pageSize = newSize;
  };

  const getPaginatedData = <T extends any>(data: T[], pageNumber?: number, pageSize?: number) => {
